...
def linear_regression_gradient_descent(X: np.ndarray, y: np.ndarray, alpha: float, iterations: int) -> np.ndarray:
    ...
    Args:
        X: np.ndarray = Feature matrix where each row represents a training example and each column represents a feature
    ...
    Returns:
        theta: np.ndarray = Optimized parameter vector (theta) rounded to 4 decimal places, where each element represents the coefficient for a feature
